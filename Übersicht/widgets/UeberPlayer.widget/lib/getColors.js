...
const getColors = ({ img }, previousState, options) => {
    ...
    let secondaryContrast = 0,
        tercaryContrast = 0;
    const primaryColorLum = luminance(
        primaryColor[0],
        primaryColor[1],
        primaryColor[2]
    );
    ...
    let palette = Thief.getPalette(img).map((swatch) => [
        ...swatch,
        luminance(...swatch),
    ]);
    palette = palette
        .filter((s) => s[3] >= 0.002 && s[3] <= 0.9)
        .concat(palette.filter((s) => s[3] < 0.002 || s[3] > 0.9));
    ...
    for (const swatch of palette) {
        ...
        if (contrastValue >= options.minContrast) {
            if (secondaryContrast < options.minContrast) {
                // Secondary color takes priority
                secondaryColor = swatch;
                secondaryContrast = contrastValue;
            } else {
                // Tercary color later and break the loop from here
                tercaryColor = swatch;
                tercaryContrast = contrastValue;
                break;
            }
        } else if (contrastValue > secondaryContrast) {
            // If contrast is below threshold, save the most contrasting colors just in case
    ...
    if (secondaryContrast < 1.75) {
        secondaryColor = getFallbackColor(secondaryColor, primaryColorLum, 0.2);
    }
    if (tercaryContrast < 1.75) {
        tercaryColor = getFallbackColor(tercaryColor, primaryColorLum, 0.3);
    }
