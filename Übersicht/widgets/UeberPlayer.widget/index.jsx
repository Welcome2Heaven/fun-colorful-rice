...
const options = {
  ...
  /* Widget position! */
  verticalPosition: "5", // -> top (default) | center | bottom | "<number>" | "-<number>"
  horizontalPosition: "40", // -> left (default) | center | right | "<number>" | "-<number>"
...
export const className = `
  font-family: "Iosevka Type",-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol";
  color: white;
...
const Progress = styled("div")`
  ...
  &::after {
    ...
    background: ${(props) =>
      props.progressColor ? props.progressColor : "white"};
...
const checkForUpdate = async (dispatch) => {
  let resp = await fetch(
    "https://raw.githubusercontent.com/aCluelessDanny/UeberPlayer/master/widget.json"
  );
  if (!resp.ok) {
    throw Error("Unable to check for update!");
  }
...
const updateNotif = (text, color = "inherit") => {
  return text ? (
    <UpdateText style={{ color }}>
      <a href="https://raw.githubusercontent.com/aCluelessDanny/UeberPlayer/master/UeberPlayer.widget.zip">
...
const big = (
  { track, artist, album, elapsed, duration },
  secondaryColor,
  tercaryColor,
  artwork,
  updateAvailable
  ...
  <BigPlayer>
    ...
    <Information>
      ...
      />
      <Track className="small" color={secondaryColor}>
        {track}
      </Track>
      <Artist className="small" color={tercaryColor}>
        {artist}
      </Artist>
    </Information>
  </BigPlayer>
...
export const render = (
  ...
  return size === "mini" ? (
    ...
    >
      {size === "big" &&
        big(song, secondaryColor, tercaryColor, artwork, updateAvailable)}
      {size === "medium" &&
        medium(song, secondaryColor, tercaryColor, artwork, updateAvailable)}
      {size === "small" &&
        small(song, secondaryColor, tercaryColor, artwork, updateAvailable)}
    </Wrapper>
  );
};
